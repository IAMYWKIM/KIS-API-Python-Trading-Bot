...
# ==========================================================
# 🚨 MODIFIED: [배포 락다운 타임라인 전면 확장 (Case 09 절대 사수)] 기존 15:12~15:31 EST로 짧게 설정되어 있던 업데이트 금지 구역을 프리장 개장부터 애프터장 정산 완료 시점(04:00 ~ 16:10 EST)까지 전면 확장. 장중 `/update` 격발로 인한 핫픽스 롤백(Rollback) 및 메모리 증발 대참사를 시스템 전역에서 원천 봉쇄.
...
class SystemUpdater:
    ...
    async def is_update_allowed(self):
        ...
        # 🚨 MODIFIED: [배포 락다운 전면 확장] 프리장(04:00)부터 정산 완료(16:10)까지 업데이트 전면 통제
        start_lock = datetime.time(4, 0)
        end_lock = datetime.time(16, 10)
        
        if start_lock <= curr_time <= end_lock:
            return False, "⚠️ <b>[배포 금지]</b> 퀀트 엔진 교전 윈도우(04:00~16:10 EST) 중에는 스케줄러 메모리 증발 및 핫픽스 롤백 방지를 위해 자동 업데이트를 강제 차단합니다. 장 마감 후 시도하십시오."
        return True, ""
